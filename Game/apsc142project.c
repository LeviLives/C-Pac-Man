...
int main(void) {
    ...
    int ghostCount = 0;
    int playerCount = 0;
    for (int i = 0; i < HARDCODED_WIDTH * HARDCODED_HEIGHT; i++)
        if (hardcoded_map[i] == GHOST) {
            ghostCount++;
        }
        else if (hardcoded_map[i]==PLAYER) {
            playerCount++;
        }
    ;

    //carrying over the global variables and constants
    map = hardcoded_map;
    ...
    //alternatively, a more restrictive condition is playerCount != 1
    //, but we may introduce multiplayer mode
    if (playerCount == 0) {
        return ERR_NO_PLAYER;
    }

    //alternatively, a more restrictive condition is ghostCount != 2
    //, but we may introduce additional ghosts
    if (ghostCount < 2) {
        return ERR_NO_GHOSTS;
    }

    //call print_map to display the map
    print_map();
